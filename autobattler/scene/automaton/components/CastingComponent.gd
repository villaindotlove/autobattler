...
func cast(spawn_pos: Vector3) -> void:
	...
	main_spell_instance.global_position = spawn_pos
	main_spell_instance.cast(spawn_pos, casting_unit)
#	_on_cast_proc.cast(spawn_pos, casting_unit)
	cast_finished.emit()
